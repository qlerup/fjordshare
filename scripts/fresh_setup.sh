...
step_2_nfs() {
  echo
  echo "Step 2/5: Optional NFS uploads mount (/etc/fstab)"
  if ask_yes_no "Configure NFS mount for uploads in /etc/fstab?" "$(is_truthy "$SETUP_NFS_UPLOADS_ENABLED" && echo y || echo n)"; then
    ...
    SETUP_NFS_MOUNT_ROOT="$(ask_input "Local NFS mount root" "$SETUP_NFS_MOUNT_ROOT" "/home/qlerup/synology/fjordshare-data" "Local mount root on Proxmox. Must be absolute.")"
    SETUP_NFS_UPLOADS_SUBDIR="$(ask_input "Uploads subdir inside NFS mount" "$SETUP_NFS_UPLOADS_SUBDIR" "uploads" "Host-side subfolder name only.")"
...
step_3_storage() {
  echo
  echo "Step 3/5: Storage paths (host paths)"
  DATA_DIR="$(ask_input "DATA_DIR (DB + internal app state)" "$DATA_DIR" "/home/qlerup/fjordshare-local/data" "Should usually be local disk.")"
  UPLOADS_HOST_DIR="$(ask_input "UPLOADS_HOST_DIR (shared files)" "$UPLOADS_HOST_DIR" "/home/qlerup/synology/fjordshare-data/uploads" "Root path for uploaded/shared files.")"
  THUMBS_HOST_DIR="$(ask_input "THUMBS_HOST_DIR (thumbnails)" "$THUMBS_HOST_DIR" "/home/qlerup/fjordshare-local/thumbs" "Root path for generated thumbnails.")"
}
...
if [ "${1:-}" = "--start-only" ]; then
  echo "==> FjordShare start-only mode"
  echo "    Repo: ${REPO_DIR}"
  echo "    Env : ${ENV_FILE}"
  if [ ! -f "$ENV_FILE" ]; then
...
echo "==> FjordShare guided setup"
echo "    Repo: ${REPO_DIR}"
echo "    Env : ${ENV_FILE}"
echo "    Tip : press Enter to use the default at each prompt."
echo "    Input examples:"
echo "      - APP_PORT: 9090 or 9080"
echo "      - DATA_DIR: /home/qlerup/fjordshare-local/data"
echo "      - UPLOADS_HOST_DIR: /home/qlerup/synology/fjordshare-data/uploads"
